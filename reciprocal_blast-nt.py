...
### First create a directory to store the analysed 
### sequences in. Then remove subdirectory where 
### the BLAST analyses were done, and move the fasta 
### file used out of the way by placing it in a new 
### directory.
def clean_directory(file):
	if not os.path.exists("Analysed_seq"):
		os.mkdir("Analysed_seq")
	shutil.rmtree(file[:-4])
	shutil.move('%s' % file, 'Analysed_seq/%s' % file)
